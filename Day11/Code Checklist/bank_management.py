...
class BankAccount:
    def __init__(self, holder: str, acc_num: str, opening_balance: Number = 0):
    ...
    def _add_transaction(self, txn_type: str, amount: Decimal):
        transaction = {"type": txn_type, "amount": amount, "balance": self._balance}
    ...
    def transfer(self, amount: Number, target_account: "BankAccount" = None) -> None:
        ...
        print(f"Transferred Rs.{amount:,.2f} " f"to {target_account.holder}")
    ...
    def mini_statement(self) -> None:
        ...
        if not self.transactions:
            ...
            for txn in self.transactions:
                ...
                print(
                    f'{txn["type"]} {sign}Rs.{txn["amount"]:.2f} Balance: Rs.{txn["balance"]:.2f}'
                )

        print(f"\nCurrent Balance: Rs.{self._balance:.2f}")

    def __str__(self) -> str:
        return f"Account Holder: {self.holder} | Account Number: {self.acc_num} | Balance: Rs.{self._balance:,.2f}"
